...
Propose first-draft specs from `reference/component-patterns/` (the ~20 canonical types — hero, feature-grid, testimonial, pricing-table, faq-accordion, cta-band, logo-cloud, team-grid, blog-card, contact-form, etc.) and let the user adjust rather than author from scratch.
...
| "This section needs a custom one-off component" | Often premature. Check `reference/component-patterns/` + `components.yaml` for a reusable existing type first. Genuinely novel = fine; near-duplicate ("HeroBlockV2" differing only in content) = merge. |
