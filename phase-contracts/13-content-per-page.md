...
**"What's a section?"** Surfaces when the user has never thought about pages as section-composed. The agent walks through 2-3 example pages from reference (`${CLAUDE_PLUGIN_ROOT}/reference-corpus/brand-examples/` if populated; otherwise context7 / WebFetch for stack-appropriate templates per `DESIGN-templates-catalog.md`'s framing: *templates as inspiration, never imported*). The user gradually recognizes that every page is a stack of named, reusable parts.
...
- `${CLAUDE_PLUGIN_ROOT}/reference-corpus/brand-examples/` — 5-8 complete brand systems showing voice + tokens + component patterns; relevant references for phase-13 brief language.
