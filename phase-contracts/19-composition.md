...
- **Reference-data load** — `reference/component-patterns/` for canonical section-composition archetypes; the chosen `component-libraries/{stack}.md` for stack-specific composition idioms. Per `DESIGN-templates-catalog.md`, templates may be surfaced as composition-pattern inspiration (how a reference site sequences a page) but are never imported — the page is composed from *this* project's components.
